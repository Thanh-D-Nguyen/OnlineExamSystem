...
class NewQuestion extends Component {
  ...
  constructor(props) {
    super(props);
    this.formRef = React.createRef(); // Tạo ref cho form
    this.state = {
      questionDetails: {
        questionimage: null,
        options: [
  ...
  handleFinishFailed = (errorInfo) => {
    ...
    // Logic to handle form failure (e.g., scroll to the first field with an error)
  };
              ...
              <Row>
                <Col span={8}>
                  <Form.Item
                    label="Subject"
                    rules={[
                      { required: true, message: "Please select any subject!" },
                    ]}
                    hasFeedback
                  >
                    <Select
